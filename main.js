...
class FormModal extends obsidian_1.Modal {
    constructor(app, title, fields, onSubmit, submitLabel = "Create") {
        super(app);
        this.values = {};
        this.setTitle(title);
        this.fields = fields;
    ...
    onOpen() {
        ...
        for (const field of this.fields) {
            const setting = new obsidian_1.Setting(contentEl).setName(field.label);
            ...
            if (field.type === "dropdown" && field.options) {
                ...
                setting.addTextArea((text) => {
                    ...
                    text.onChange((value) => (this.values[field.key] = value));
                });
            }
            else {
...
class ChoiceModal extends obsidian_1.Modal {
    constructor(app, title, message, choices, onChoose) {
        super(app);
        this.setTitle(title);
        const { contentEl } = this;
        contentEl.createEl("p", { text: message });
        for (const choice of choices) {
            new obsidian_1.Setting(contentEl).addButton((button) => {
...
class PhDApplicationHubPlugin extends obsidian_1.Plugin {
    constructor() {
        super(...arguments);
        this.settings = DEFAULT_SETTINGS;
        this.applicationDeadlines = new Map();
        this.deadlineSyncInProgress = new Set();
    }
    async onload() {
    ...
    get root() {
        return (0, obsidian_1.normalizePath)(this.settings.workspaceFolder.trim() || "PhD Application");
    }
    path(relative) {
        return (0, obsidian_1.normalizePath)(`${this.root}/${relative}`);
    }
    async loadSettings() {
    ...
    async initializeWorkspace() {
        ...
        new obsidian_1.Notice("PhD Application Hub workspace is ready.");
    ...
    async writeOrReplace(filePath, content) {
        await this.ensureFolder(filePath.split("/").slice(0, -1).join("/"));
    ...
    activeFileOfType(type) {
        const file = this.app.workspace.getActiveFile();
        if (!file) {
            new obsidian_1.Notice(`Open a ${type} note first.`);
            return null;
        }
        const fm = this.app.metadataCache.getFileCache(file)?.frontmatter;
        if (fm?.type !== type) {
            new obsidian_1.Notice(`This command must be run inside a ${type} note.`);
            return null;
        }
        return file;
    }
    openCreateSupervisorModal() {
        void this.ensureWorkspace().then(() => {
            new FormModal(this.app, "Add supervisor", [
                ...
                if (!values.name.trim())
                    return void new obsidian_1.Notice("Supervisor name is required.");
                await this.createSupervisor(values);
            }).open();
        });
    }
    async createSupervisor(values) {
        const name = values.name.trim();
        const filePath = this.path(`10 Supervisors/${this.safeFileName(name)}.md`);
        if (this.app.vault.getAbstractFileByPath(filePath))
            return void new obsidian_1.Notice("That supervisor already exists.");
        ...
        await this.app.vault.create(filePath, content);
        new obsidian_1.Notice(`Created supervisor: ${name}`);
    }
    async openUpdateSupervisorStatusModal() {
        const file = this.activeFileOfType("supervisor");
        if (!file)
            return;
        const fm = this.app.metadataCache.getFileCache(file)?.frontmatter ?? {};
        const options = this.toOptions(CONTACT_STATUSES);
        new FormModal(this.app, "Update supervisor contact status", [
            ...
            new obsidian_1.Notice(`Contact status → ${values.status}`);
            if (values.status === "Positive")
                this.offerCreateUniversityAndApplication(file);
        }, "Update").open();
    }
    offerCreateUniversityAndApplication(supervisorFile) {
        new ChoiceModal(this.app, "Positive response", "A Positive response normally means you now plan to apply. Create the linked university and application?", [
    ...
    async createUniversityFromSupervisor(supervisorFile, createApplicationAfter) {
        ...
        new FormModal(this.app, "Create university", [
            ...
            if (!values.name.trim())
                return void new obsidian_1.Notice("University name is required.");
            const universityName = values.name.trim();
            await this.ensureUniversity(values, supervisorFile.basename);
            await this.app.fileManager.processFrontMatter(supervisorFile, (frontmatter) => {
                frontmatter.university = `[[${universityName}]]`;
            });
            if (createApplicationAfter)
                this.openCreateApplicationModal(universityName, supervisorFile.basename, fm.priority ?? "B");
        }).open();
    }
    openCreateUniversityModal() {
        void this.ensureWorkspace().then(() => {
            new FormModal(this.app, "Add university", [
    ...
    async ensureUniversity(values, supervisorName = "") {
        const name = values.name.trim();
        const filePath = this.path(`20 Universities/${this.safeFileName(name)}.md`);
        const existing = this.app.vault.getAbstractFileByPath(filePath);
        if (existing instanceof obsidian_1.TFile) {
            new obsidian_1.Notice(`University already exists: ${name}`);
            return existing;
        }
        ...
        new obsidian_1.Notice(`Created university: ${name}`);
        return created;
    }
    openCreateApplicationModal(university = "", supervisor = "", priority = "B") {
        void this.ensureWorkspace().then(() => {
            const statusOptions = this.toOptions(APPLICATION_STATUSES);
            new FormModal(this.app, "Add application", [
                ...
                if (!values.university.trim())
                    return void new obsidian_1.Notice("University is required.");
    ...
    async createApplication(values) {
        ...
        if (this.app.vault.getAbstractFileByPath(filePath))
            return void new obsidian_1.Notice("That application already exists.");
        const taskLines = APPLICATION_TASK_SCHEDULE.map(({ text, daysBefore }) => `- [ ] ${text}${deadline ? ` 📅 ${this.shiftDate(deadline, -daysBefore)}` : ""}`).join("\n");
    ...
    openUpdateApplicationDeadlineModal() {
        ...
        new FormModal(this.app, "Update application deadline", [
            ...
            if (deadline && !this.isDate(deadline)) {
                new obsidian_1.Notice("Use YYYY-MM-DD for the deadline.");
                return;
            }
            ...
            new obsidian_1.Notice(deadline ? `Deadline → ${deadline}; checklist rescheduled.` : "Deadline cleared; managed task due dates removed.");
        }, "Update").open();
    }
    async captureAndSyncApplicationDeadlines() {
        const applicationFolder = `${this.path("30 Applications")}/`;
        for (const file of this.app.vault.getMarkdownFiles()) {
            if (!file.path.startsWith(applicationFolder))
                continue;
            const fm = this.app.metadataCache.getFileCache(file)?.frontmatter;
    ...
    async migrateGeneratedFiles() {
        const applicationsBase = this.app.vault.getAbstractFileByPath(this.path("01 Bases/Applications.base"));
        if (applicationsBase instanceof obsidian_1.TFile) {
            await this.app.vault.process(applicationsBase, (content) => content.replace('days_left: \'if(deadline, ((deadline - today()) / 86400000).round(), null)\'', 'days_left: \'if(deadline, ((number(deadline) - number(today())) / 86400000).round(), null)\''));
        }
    }
    async handleApplicationMetadataChanged(file) {
        const applicationFolder = `${this.path("30 Applications")}/`;
        if (!file.path.startsWith(applicationFolder) || this.deadlineSyncInProgress.has(file.path))
            return;
        const fm = this.app.metadataCache.getFileCache(file)?.frontmatter;
        if (fm?.type !== "application")
            return;
        ...
        if (previousDeadline === rawDeadline)
            return;
        this.applicationDeadlines.set(file.path, rawDeadline);
        if (rawDeadline && !this.isDate(rawDeadline)) {
            new obsidian_1.Notice(`Invalid application deadline in ${file.basename}. Use YYYY-MM-DD.`);
            return;
        }
        await this.syncApplicationDeadline(file, rawDeadline);
        new obsidian_1.Notice(rawDeadline ? `Deadline changed; checklist rescheduled for ${file.basename}.` : `Deadline cleared; task due dates removed for ${file.basename}.`);
    }
    async syncApplicationDeadline(file, deadline) {
        if (this.deadlineSyncInProgress.has(file.path))
            return;
    ...
    replaceManagedTaskDueDate(content, taskText, dueDate) {
        const escaped = taskText.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const pattern = new RegExp(`^(\\s*[-*+]\\s+\\[[^\\]]\\]\\s+${escaped})(?:\\s+📅\\s+\\d{4}-\\d{2}-\\d{2})?(.*)$`, "m");
        return content.replace(pattern, (_match, prefix, suffix) => {
    ...
    async openUpdateApplicationStatusModal() {
        const file = this.activeFileOfType("application");
        if (!file)
            return;
        const fm = this.app.metadataCache.getFileCache(file)?.frontmatter ?? {};
        const options = this.toOptions(APPLICATION_STATUSES);
        new FormModal(this.app, "Update application status", [{ key: "status", label: "Application status", type: "dropdown", value: APPLICATION_STATUSES.includes(fm.status) ? fm.status : "Preparing", options }], async (values) => {
            await this.app.fileManager.processFrontMatter(file, (frontmatter) => {
                frontmatter.status = values.status;
                if (values.status === "Submitted" && !frontmatter.submitted_date)
                    frontmatter.submitted_date = this.today();
            });
            new obsidian_1.Notice(`Application status → ${values.status}`);
    ...
    openSimpleFrontmatterFieldModal(file, title, key, value, placeholder) {
        new FormModal(this.app, title, [{ key, label: title, value, placeholder }], async (values) => {
            await this.app.fileManager.processFrontMatter(file, (frontmatter) => {
    ...
    openCreateInteractionModal() {
        void this.ensureWorkspace().then(() => {
            new FormModal(this.app, "Record interaction", [
                ...
                if (!values.subject.trim())
                    return void new obsidian_1.Notice("Subject is required.");
    ...
    async createInteraction(values) {
        ...
        await this.app.vault.create(filePath, content);
        new obsidian_1.Notice("Interaction recorded.");
    }
    openCreatePaperModal() {
        void this.ensureWorkspace().then(() => {
            new FormModal(this.app, "Add relevant paper", [
                ...
                if (!values.title.trim())
                    return void new obsidian_1.Notice("Paper title is required.");
                await this.createPaper(values);
            }).open();
        });
    }
    async createPaper(values) {
        const supervisors = this.parseList(values.supervisors);
        ...
        await this.app.vault.create(filePath, content);
        new obsidian_1.Notice("Paper note created.");
    }
    openCreateDocumentModal() {
        void this.ensureWorkspace().then(() => {
            new FormModal(this.app, "Add application document record", [
                ...
                if (!values.name.trim())
                    return void new obsidian_1.Notice("Document name is required.");
    ...
    async createDocument(values) {
        ...
        await this.app.vault.create(filePath, content);
        new obsidian_1.Notice("Document record created.");
    }
    async validateWorkspace() {
        ...
        for (const file of this.app.vault.getMarkdownFiles()) {
            ...
            if (!fm)
                continue;
            if (fm.type === "application" && !APPLICATION_STATUSES.includes(fm.status))
                problems.push(`${file.path}: invalid application status "${fm.status ?? ""}"`);
            if (fm.type === "supervisor" && !CONTACT_STATUSES.includes(fm.contact_status))
                problems.push(`${file.path}: invalid contact status "${fm.contact_status ?? ""}"`);
        }
        if (!problems.length)
            return void new obsidian_1.Notice("Workspace validation passed.");
        new obsidian_1.Notice(`Found ${problems.length} workspace issue(s). See developer console for details.`, 8000);
...
exports.default = PhDApplicationHubPlugin;
class PhDApplicationHubSettingTab extends obsidian_1.PluginSettingTab {
    ...
    display() {
        ...
        new obsidian_1.Setting(containerEl)
            ...
            .addText((text) => text.setValue(this.plugin.settings.cycle).onChange(async (value) => {
        ...
        new obsidian_1.Setting(containerEl)
            ...
            .addText((text) => text.setValue(this.plugin.settings.defaultProgram).onChange(async (value) => {
        ...
        new obsidian_1.Setting(containerEl)
            ...
            .addButton((button) => button.setButtonText("Initialize workspace").setCta().onClick(() => {
